<div class="w-full text-gray-700 bg-white">
    <div x-data="{ open: false }"
        ...
        <nav :class="{'flex': open, 'hidden': !open}"
            ...
            <a class="{{$currentUrl == route('libros.index') ? 'bg-gray-200' : ''}} px-4 py-2 mt-2 text-sm font-semibold text-gray-900 rounded-lg md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                href="{{route('libros.index')}}">Libros
            </a>
            <a class="{{$currentUrl == route('prestamos.index', ['user_id' => auth()->id()]) ? 'bg-gray-200' : ''}} px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                href="{{route('prestamos.index', ['user_id' => auth()->id()])}}">
                Prestamos
            </a>
            <a class="{{$currentUrl == route('carrito.index') ? 'bg-gray-200' : ''}} relative px-4 py-2 mt-2 md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                href="{{route('carrito.index')}}">
                <svg viewBox="0 -31 512.00026 512" xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 fill-current">
                    <path
                ...
                <div class="absolute top-0 right-0 w-5 h-5 flex items-center justify-center rounded-full bg-blue-700">
                    <span class="text-xs text-white">{{ $cartTotal }}</span>
                </div>
            </a>
            <div @click.away="open = false" class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="px-2 py-2 bg-white rounded-md shadow">
                        <a class="{{$currentUrl == route('perfiles.show', ['id' => auth()->id()]) ? 'bg-gray-200' : ''}} block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg  md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                            href="{{route('perfiles.show', ['id' => auth()->id()])}}">
                            Perfil
                        </a>
